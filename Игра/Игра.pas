...
Uses Справка, Результат, ВСтатистику;
...
Form3.RecordResult.Lines.LoadFromFile(path+'\ПоОчкам.txt');
b:=false;
NumberStringRecord:=0;
 ...
 while i<>0  do begin
 j:=0;
   while copy(Form3.RecordResult.Lines[i-1],j,1)<>':' do begin j:=j+1; end; //Ищем начало.
   j:=j+2;//Начало.
   j1:=j;
   while copy(Form3.RecordResult.Lines[i-1],j1,1)<>' ' do begin j1:=j1+1; end;   //Ищем конец.
   //j1:=j1-1;//Конец.
   long:=j1-j;
...
Form3.RecordlongWay.Lines.LoadFromFile(path+'\СамаяДлиннаяЦепочка.txt');
k:=1;
b:=false;
NumberStringRecord:=0;
 ...
 j:=length(Form3.RecordlongWay.Lines[i-1]);
   while copy(Form3.RecordlongWay.Lines[i-1],j,1)<>':' do begin  j:=j-1; end; //Ищем начало.
   j:=j+1;//Начало.
...
if Win=true then  //Если полностью очистил поле.
 begin
  color:=clYellow;
  Finish:='Поздравяем, вы выиграли!(+5000 за победу)';
  Score:=Score+5000;
  masrecord[1]:=1;
  end else
    begin
...
if b=false then  //Если не рекорд, но строк в таблице рекордов меньше 10, то добавляем рекорд.
  begin
...
if win=true then begin b:=true; end;                           //Если выиграл.
if RecordScore(Score,masrecord)=true then begin b:=true; end;   //Если рекорд по очкам
if RecordWay(MaxLongWay,masrecord)=true  then begin b:=true; end;  //Если рекорд по длине цепочки.
...
var color:Tcolor;
begin
   label2.caption:= inttostr(Score);   //Выводим на экран очки.
   label4.Caption:=('+'+inttostr(Score-Score1-NumberDeleteRectangle)); //Выводим на экран бонусы.
  ...
  history1.caption:=inttostr(NumberDeleteRectangle)+' квадрата(ов)';
...
begin
  b:=false;
  Clear:=true; //Определяем, остались ли кубы.
...
for I := 0 to 20 do begin            //Сдвигаем строки, стоящние выше удаляемых кубиков.
 for j := 0 to 20 do begin
  ...
  if NumberDeleteRectangle>MaxLongWay then begin MaxLongWay:=NumberDeleteRectangle end;  //Максимальная длина цепочки.
  oldcolor:= SetOldColor(pole[Y,X]);  //Цвет, На который указывает указатель.
  Result(NumberDeleteRectangle,Score,Score1,history1,history2,history3,history4,history5,label2,label4); //Результат хода.

  if Gameover(pole,Win)=false then begin ShowResult(Score,MaxLongWay,Win,masRecord); end;  //Конец игры.
...
case Key of
 ...
 VK_RETURN: begin
            ...
            DeleteRectangle (X,Y,NumberClearX,MaxLongWay,color,oldcolor,Image1,pole,masRecord,Score,Win,Label2,Label4,history1,history2,history3,history4,history5);     //Удаление.
            end;
            end;
...
X:=0;
Y:=0;
NumberClearX:=0; //Количество пустых столбов.
Score:=0;        //Очки
MaxLongWay:=0;   //Максимальная длинна цепочки.
Win:=false;      //Победил или нет.
